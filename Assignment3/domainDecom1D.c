...
int main(int argc, char *argv[]){
    ...
    //-----------------------------------------------------------------------------
    if(rank !=0)
        MPI_Isend(f+1, 1, MPI_DOUBLE, rank-1, 0, MPI_COMM_WORLD, & request1);
    else
        MPI_Isend(f+1, 1, MPI_DOUBLE, size-1, 0, MPI_COMM_WORLD, &request1);

    if(rank !=size-1)
        MPI_Isend(f + nxn_loc - 2, 1, MPI_DOUBLE, rank + 1, 1, MPI_COMM_WORLD, &request2);
    else
        MPI_Isend(f + nxn_loc - 2, 1, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD, &request2);
    //-----------------------------------------------------------------------------
    ...
    if(rank != size-1)
        MPI_Irecv(&right_ghost_cell_recv, 1, MPI_DOUBLE, rank+1, 0, MPI_COMM_WORLD, &request3);
    else
        MPI_Irecv(&right_ghost_cell_recv, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &request3);
    MPI_Wait(&request3, &status);

    if (rank != 0)
        MPI_Irecv(&left_ghost_cell_recv, 1, MPI_DOUBLE, rank - 1, 1, MPI_COMM_WORLD, &request4);
    else
        MPI_Irecv(&left_ghost_cell_recv, 1, MPI_DOUBLE, size - 1, 1, MPI_COMM_WORLD, &request4);
